...
test('GET /api/internal/metrics/alerts/rules returns rule list (7 rules) + names', async () => {
  const app = createApp();
  const res = await request(app).get('/api/internal/metrics/alerts/rules');
  assert.strictEqual(res.statusCode, 200);
  assert.strictEqual(res.body.code, 0);
  assert.ok(res.body.data);
  assert.strictEqual(res.body.data.count, 7);
  assert.ok(Array.isArray(res.body.data.names));
  assert.strictEqual(res.body.data.names.length, 7);
  ...
  assert.ok(res.body.data.rules.length === 7);
  assert.ok(res.body.data.thresholds);
});

test('GET /api/internal/metrics/alerts returns fired:[] + checked:7 initially', async () => {
  const app = createApp();
  const res = await request(app).get('/api/internal/metrics/alerts');
  assert.strictEqual(res.statusCode, 200);
  assert.strictEqual(res.body.code, 0);
  assert.ok(res.body.data);
  assert.strictEqual(res.body.data.checked, 7);
